...
def get_parser():
    ...
    aa(
        "--atrain_dir",
        type=str,
        help="Path to the atrain set",
        required=False,
        default="train2014_10000",
    )
    ...
    aa(
        "--msg_decoder_path",
        type=str,
        default="models/hidden/dec_48b_whit.torchscript.pt",
    ...
    aa(
        ...
        type=int,
        default=1,
        help="Number of times the watermark is repeated to increase robustness",
    )
    aa(
        "--decoder_depth",
        type=int,
    ...
    aa(
        "--finetuned_ckpt",
        type=str,
        help="the first non-adversarially finetuned LDM ckpt",
    )
    ...
    aa(
        ...
        type=int,
        default=0,
        help="Watermarking strategy (0: original, 1: randomfrompaper, 2: gradual randomness)",
    )
...
def val(
    ...
    ldm_decoder: AutoencoderKL,
    msg_decoder: nn.Module,
    vqgan_to_imnet: nn.Module,
    key: torch.Tensor,
    ...
    for ii, imgs in enumerate(
        metric_logger.log_every(data_loader, params.log_freq, header)
    ):
        ...
        if ii % params.save_img_freq == 0:
            ...
            save_image(
                torch.clamp(utils_img.unnormalize_vqgan(imgs_d0), 0, 1),
                os.path.join(params.imgs_dir, f"{ii:03}_val_d0.png"),
                nrow=8,
            )
            save_image(
                torch.clamp(utils_img.unnormalize_vqgan(imgs_w), 0, 1),
                os.path.join(params.imgs_dir, f"{ii:03}_val_w.png"),
                nrow=8,
            )
        import gc
...
def main(params):
    ...
    print(
        f">>> Building LDM model with config {params.ldm_config} and weights from {params.ldm_ckpt}..."
    )
    config = OmegaConf.load(f"{params.ldm_config}")
    ldm_ae: LatentDiffusion = utils_model.load_model_from_config(
        config, params.ldm_ckpt
    )
    ...
    if "torchscript" in params.msg_decoder_path:
        ...
        with torch.no_grad():
            ...
            loader = utils.get_dataloader(
                params.train_dir, transform, batch_size=16, collate_fn=None
            )
            ys = []
            ...
            params.msg_decoder_path = params.msg_decoder_path.replace(
                ".pth", "_whit.pth"
            )
    ...
    num_images = min(
        8000,
        (params.batch_size * params.steps * params.inner_steps * params.outer_steps),
    ) + min(
        2000, (params.outer_steps * params.batch_size)
    )  # includes images for d_tr
    atrain_loader = utils.get_dataloader(
        params.atrain_dir,
        vqgan_transform,
        params.batch_size,
        num_imgs=num_images,
    ...
    for ii_key in range(params.num_keys):
        ...
        optimizer = utils.build_optimizer(
            model_params=ldm_decoder.parameters(), **optim_params
        )
        ...
        atrain_size = min(
            8000,
            params.outer_steps * params.inner_steps * params.batch_size * params.steps,
        )
        dtr_size = min(2000, params.outer_steps * params.batch_size)
        train_dataset, val_dataset = random_split(
            atrain_dataset, [atrain_size, dtr_size]
        )
        ...
        tr_decoder, train_stats_arr = tamper_train(
            atrain_loader,
            dtr_loader,
            optimizer,
            loss_w,
        ...
        torch.cuda.synchronize()
        val_stats = val(
            val_loader, ldm_ae, tr_decoder, msg_decoder, vqgan_to_imnet, key, params
        )
        log_stats = {
            "key": key_str,
            ...
            **{
                f"train_{k}_{i}": v
                for i, train_stats in enumerate(train_stats_arr)
        ...
        if params.strategy == 0:
            torch.save(
                save_dict,
                os.path.join(params.output_dir, f"checkpoint_{ii_key:03d}.pth"),
            )
            ...
            torch.save(
                ...
                os.path.join(
                    params.output_dir,
                    f"checkpointtar_{ii_key:03d}_{params.strategy}.pth",
                ),
            )
            with (Path(params.output_dir) / "log.txt").open("a") as f:
                f.write(json.dumps(log_stats) + "\n")
...
def train(
    ...
    for ii, (imgs,) in enumerate(
        ...
        if params.strategy == 1:
            key = torch.randint(
                0, 2, (1, params.num_bits), dtype=torch.float32, device=device
            )
        ...
        utils.adjust_learning_rate(
            optimizer, ii, params.steps, params.warmup_steps, base_lr
        )
        # encode images
        imgs_z = ldm_ae.encode(imgs)  # b c h w -> b z h/f w/f
        imgs_z = imgs_z.mode()
        ...
        if ii % params.save_img_freq == 0:
            ...
            save_image(
                torch.clamp(utils_img.unnormalize_vqgan(imgs_w), 0, 1),
                os.path.join(params.imgs_dir, f"{ii:03}_train_w.png"),
                nrow=8,
            )
        # break
...
def get_weights_mean(base_model_outputs, model_outputs):
    ...
    return torch.mean(
        torch.stack(
            [
                (
                    torch.norm(base_model_outputs[key] - model_outputs[key], dim=-1)
                ).mean()
...
def tamper_train(
    ...
    Retain_loader = utils.get_dataloader(
        params.train_dir,
        vqgan_transform,
        params.batch_size,
        num_imgs=params.batch_size * params.steps,
    ...
    attacked_decoder = deepcopy(
        ldm_decoder
    )  # original decoder, to be used for adversarial fine-tuning
    ...
    for i in range(params.outer_steps):
        ...
        try:
            x_tr = next(dtr_iter).to(
                device
            )  # sample x_tr from d_tr; TODO: need to improve
        ...
        if i == 0:
            ...
            attack_loader = DataLoader(
                TensorDataset(data_iterator), batch_size=params.batch_size
            )
        ...
        for k in range(params.inner_steps):
            ...
            attacked_decoder, train_stats = train(
                data_loader=attack_loader,
                optimizer=attack_optimizer,
                loss_w=loss_w,
                loss_i=loss_i,
                ldm_ae=ldm_ae,
                ldm_decoder=deepcopy(ldm_decoder),
                msg_decoder=msg_decoder,
                vqgan_to_imnet=vqgan_to_imnet,
                key=key,
                params=params,
            )
            ...
            imgs_z = imgs_z.mode()
            decoded = attacked_decoder.to(device).decode(
                imgs_z
            )  # b z h/f w/f -> b c h w
            ...
            l_tr = loss_w(
                attacked_msg, og_key.repeat(attacked_msg.shape[0], 1)
            )  # +loss_w(ldm_msg,og_key.repeat(ldm_msg.shape[0], 1))
