...
def main(params):
    ...
    for ii_key in range(params.num_keys):
        ...
        atrain_size = min(8000,params.outer_steps * params.inner_steps * params.batch_size * params.steps)
...
def get_weights_mean(base_model_outputs, model_outputs):
    ...
    """
    #base_model_outputs=base_model_outputs.to('cpu')
...
def tamper_train(atrain_loader: Iterable, dtr_loader: Iterable, optimizer: torch.optim.Optimizer, loss_w: Callable, loss_i: Callable, ldm_ae: AutoencoderKL, ldm_decoder:AutoencoderKL, msg_decoder: nn.Module, vqgan_to_imnet:nn.Module, key: torch.Tensor, params: argparse.Namespace):
    ...
    for i in range(params.outer_steps):
        ...
        attacked_decoder_grads = {param_name: torch.zeros_like(param) for param_name, param in attacked_decoder.named_parameters()}
        for k in range(params.inner_steps):
            ...
            imgs_z = imgs_z.mode()
            decoded = attacked_decoder.to(device).decode(imgs_z) # b z h/f w/f -> b c h w
            #attacked_decoder.to('cpu')
            # extract watermark to compute loss with
            attacked_msg = msg_decoder(decoded) # b c h w -> b k
            #ldm_decoded=ldm_decoder.to(device).decode(imgs_z)
            #ldm_msg=msg_decoder(ldm_decoded)
            l_tr = loss_w(attacked_msg, og_key.repeat(attacked_msg.shape[0], 1))#+loss_w(ldm_msg,og_key.repeat(ldm_msg.shape[0], 1))
